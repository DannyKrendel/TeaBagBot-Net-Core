...
namespace TeaBagBot.DataAccess
{
    public class MongoRepository<TEntity> : IRepository<TEntity> where TEntity : IEntity
    {
        ...
        {
            return ((BsonCollectionAttribute)(typeof(TEntity)).GetCustomAttributes(typeof(BsonCollectionAttribute), true)
        ...
        {
            var filter = Builders<TEntity>.Filter.Eq(doc => doc.Id, new ObjectId(id));
