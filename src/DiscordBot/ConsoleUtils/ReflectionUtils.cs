...
namespace DiscordBot.ConsoleUtils
{
    ...
    {
        internal static T CreateObject<T>(TypeInfo typeInfo, ConsoleCommandService commands, IServiceProvider services = null)
            => CreateBuilder<T>(typeInfo, commands)(services);
        internal static Func<IServiceProvider, T> CreateBuilder<T>(TypeInfo typeInfo, ConsoleCommandService commands)
        {
        ...
        private static object GetMember(ConsoleCommandService commands, IServiceProvider services, Type memberType, TypeInfo ownerType)
        {
            if (memberType == commands.GetType())
                return commands;
